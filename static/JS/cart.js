...
const subtotalElem2 = document.querySelector('#subtotal2');
...
const totalCalc = function () {
  ...
  for (let i = 0; i < quantityElem.length; i++) {
    ...
    subtotal2 += Number(quantityElem[i].textContent) * Number(priceElem[i].textContent);

  }

  // show the `subtotal` variable value on `subtotalElem` element
  subtotalElem.textContent = subtotal.toFixed(2);
  subtotalElem2.textContent = subtotal.toFixed(2);
